...
from .auth0_admin import Auth0AdminClient, Auth0AdminError
...
mcp = MCPServer(
    ...
    instructions=(
        ...
        "github_user_profile). When configured, it can also perform narrowly "
        "scoped server-side Auth0 Management API operations. File paths are "
        "always relative to the workspace root; attempts to escape it are rejected."
    ),
    auth=_auth_settings,
...
@mcp.tool()
async def auth0_list_applications(page: int = 0, per_page: int = 100) -> list[dict]:
    """List Auth0 applications using Conduit's server-side Management API credentials."""
    if auth0_admin is None:
        raise RuntimeError("Auth0 Management API is not configured on this Conduit deployment")
    try:
        return await auth0_admin.list_clients(page=page, per_page=per_page)
    except Auth0AdminError as exc:
        raise RuntimeError(str(exc)) from exc


@mcp.tool()
async def auth0_get_application(client_id: str) -> dict:
    """Get one Auth0 application by client ID using Conduit's server-side credentials."""
    if auth0_admin is None:
        raise RuntimeError("Auth0 Management API is not configured on this Conduit deployment")
    if not client_id.strip():
        raise ValueError("client_id must not be empty")
    try:
        return await auth0_admin.get_client(client_id.strip())
    except Auth0AdminError as exc:
        raise RuntimeError(str(exc)) from exc
...
def workspace_tree() -> str:
    ...
    files = sorted(str(p.relative_to(workspace.root)) for p in workspace.root.rglob("*"))
    files = [f for f in files if (workspace.root / f).is_file()]
...
async def health(request: Request) -> JSONResponse:
    return JSONResponse({
        ...
        "server": mcp.name,
        "version": mcp.version,
        "auth0_management_api": "configured" if auth0_admin else "not_configured",
    })
...
def _render_status_page(tools, prompts, host: str, port: int) -> str:
    ...
    admin_note = (
        '<div class="node accent"><span class="label">Admin API</span><span class="value">Auth0 Management API configured</span></div>'
        if auth0_admin else '<div class="node"><span class="label">Admin API</span><span class="value">not configured</span></div>'
    )
